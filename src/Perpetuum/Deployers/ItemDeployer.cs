...
namespace Perpetuum.Deployers
{
    public abstract class ItemDeployerBase : Item
    {
        ...
        protected EntityDefault DeployableItemEntityDefault => ED.Config.TargetEntityDefault;
    }
    ...
    public class ItemDeployer : ItemDeployerBase
    {
        ...
        {
            Position spawnPosition = player.CurrentPosition.Center;
            Unit deployableItem = CreateDeployableItem(zone, spawnPosition, player);

            ErrorCodes error = CanDeploy(zone, deployableItem, spawnPosition, player);
        ...
        {
            Unit unit = (Unit)_entityServices.Factory.CreateWithRandomEID(DeployableItemEntityDefault);
