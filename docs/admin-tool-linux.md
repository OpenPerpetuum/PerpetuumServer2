...
and the definition-to-template relation. Seasons and reward packages are also
native: package contents and usage, general season settings, all activity rates,
objectives and daily pools, tier and leaderboard rewards, participation
statistics, and the six-step new-season wizard share the Windows SQL model.
Portable dependency-aware exports for existing items, robots, and seasons are
generated by the same core implementation in both front ends. This establishes the complete Linux
...
1. launch the self-contained executable and confirm the window, text, and all
   fourteen module tabs render at the workstation's actual DPI;
...
6. seasons, packages, remaining creation dialogs, and dependency-aware content
   export (all native now).

The native application has reached module and content-workflow parity with the
WPF application. WPF remains a supported compatibility front end; new database
behavior belongs in the portable core and is consumed by both front ends rather
