...
namespace Perpetuum.RequestHandlers.Characters
{
    public class CharacterRename : IRequestHandler
    {
        ...
        public void HandleRequest(IRequest request)
        {
            ...
            Account account = _accountManager.Repository.Get(request.Session.AccountId).ThrowIfNull(ErrorCodes.AccountNotFound);

            string nick = request.Data.GetOrDefault<string>(k.nick).Trim();
            Character targetCharacter = Character.Get(request.Data.GetOrDefault<int>(k.characterID));
            ...
            //withdrw credit
            MtProduct product = _mtProductHelper.GetByAccountTransactionType(AccountTransactionType.CharacterRename);
            IAccountWallet wallet = _accountManager.GetWallet(account, AccountTransactionType.CharacterRename);
            wallet.Balance -= product.price;

            AccountTransactionLogEvent e = new(account, AccountTransactionType.CharacterRename)
            {
            ...
            string oldNick = targetCharacter.Nick;
