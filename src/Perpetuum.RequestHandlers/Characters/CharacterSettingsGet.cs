...
namespace Perpetuum.RequestHandlers.Characters
{
    public class CharacterSettingsGet : IRequestHandler
    {
        public void HandleRequest(IRequest request)
        {
            ...
            string dataStr = Db.Query().CommandText("select settingsstring from charactersettings where characterid=@characterID")
                .SetParameter("@characterID", character.Id)
                .ExecuteScalar<string>();

            Dictionary<string, object> result = GenxyConverter.Deserialize(dataStr);
