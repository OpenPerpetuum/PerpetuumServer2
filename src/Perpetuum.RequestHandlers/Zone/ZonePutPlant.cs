...
namespace Perpetuum.RequestHandlers.Zone
{
    public class ZonePutPlant : IRequestHandler<IZoneRequest>
    {
        ...
        {
            int x = request.Data.GetOrDefault<int>(k.x);
            int y = request.Data.GetOrDefault<int>(k.y);
            int plantIndex = request.Data.GetOrDefault<int>(k.index);
            int plantState = request.Data.GetOrDefault<int>(k.state);
            ...
            PlantRule? rule = request.Zone.Configuration.PlantRules.FirstOrDefault(r => r.Type == (PlantType)plantIndex);
            if (rule == null)
            {
