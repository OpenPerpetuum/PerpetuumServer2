...
namespace Perpetuum.Zones
{
    public class ZoneSession : IZoneSession
    {
        ...
        private void HandleLockTerrain(Packet packet)
        {
            ...
            Position location = new(x + 0.5, y + 0.5, z);

            bool isPrimary = packet.ReadByte() != 0;

            WritePacketLog(packet, $"target = {location} primary = {isPrimary}");
            TerrainLock terrainLock = new(_player, location) { Primary = isPrimary };
        ...
        private void HandleGetTerrainLockParameters(Packet packet)
        {
            ...
            TerrainLockParametersPacketBuilder builder = new(terrainLock);
        ...
        private void HandleDeployItem(Packet packet)
        {
            ...
            BinaryStream binaryStream = new(packet.ReadBytes(argsCount));
        ...
        private void HandleGangDoodle(Packet packet)
        {
            ...
            using Packet doodlePacket = new(ZoneCommand.GangDoodle);
        ...
        private void HandleLocalChat(Packet packet)
        {
            ...
            using Packet chatPacket = new(ZoneCommand.LocalChat);
        ...
        private void HandlePutLoot(Packet packet)
        {
            ...
            List<KeyValuePair<long, int>> items = [];
        ...
        private void HandleTakeLoot(Packet packet)
        {
            ...
            List<KeyValuePair<Guid, int>> items = [];
        ...
        private void HandleUnloadAmmo(Packet packet)
        {
            ...
            if (component?.GetModule(slot) is not ActiveModule module)
            {
                return;
            }
        ...
        {
            RobotInfoPacketBuilder builder = new(_player);
            _player.Session.SendPacket(builder);
        }
        ...
        {
            Packet packet = new(ZoneCommand.StartLogout);
        ...
        {
            Packet packet = new(ZoneCommand.CancelLogout);
            SendPacket(packet);
        }
        ...
        private class BeamsMonitor : Observer<Beam>
        {
            ...
            private readonly ConcurrentQueue<Beam> _beams = new();
