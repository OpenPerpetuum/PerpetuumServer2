...
namespace Perpetuum.Services.MissionEngine.MissionProcessorObjects
{
    public partial class MissionProcessor
    {
        ...
        {
            Dictionary<string, object> originalData = data;
            Character character = Character.Get(data.GetOrDefault<int>(k.characterID));
            bool explicitGangHandling = data.GetOrDefault<int>(k.useGang) == 1;
            MissionTargetType targetType = data.GetOrDefault<MissionTargetType>(k.type);
        ...
        private void MissionTargetAdvanced(Character character, MissionTargetType targetType, bool explicitGangHandling, Dictionary<string, object> originalData)
        {
            ...
            IEnumerable<Character> others = GetGangMembersCached(character).Where(m => m.Id != character.Id);
            foreach (Character? other in others)
            {
                ...
                Dictionary<string, object> info = originalData.Clone();
        ...
        {
            Dictionary<string, object> info = (Dictionary<string, object>)infoDictionary;

            int missionId = info.GetOrDefault<int>(k.missionID);
            MissionTargetType targetType = info.GetOrDefault<MissionTargetType>(k.type);
            bool isComplete = info.GetOrDefault<bool>(k.completed);
            Character character = Character.Get(info.GetOrDefault<int>(k.characterID));
            int targetId = info.GetOrDefault<int>(k.targetID);
            Guid missionGuid = info.GetOrDefault<Guid>(k.guid);

            bool processById =
                    targetType is not (MissionTargetType.dock_in or
                    MissionTargetType.prototype or
                    MissionTargetType.massproduce or
                    MissionTargetType.research or
...
;
        ...
        private void MissionTargetAdvanceByType(Character character, MissionTargetType targetType, bool isComplete, Dictionary<string, object> data)
        {
            ...
            List<KeyValuePair<MissionTargetInProgress, MissionInProgress>> kvpList = GetTargetInProgress_and_missionInProgressByTargetType(character, targetType);
            ...
            foreach (KeyValuePair<MissionTargetInProgress, MissionInProgress> pair in kvpList)
            {
                MissionTargetInProgress missionTargetInProgress = pair.Key;
                MissionInProgress missionInProgress = pair.Value;
        ...
        private void ProcessAdvamcementForOneTarget(MissionTargetInProgress missionTargetInProgress, MissionInProgress missionInProgress, IDictionary<string, object> info, bool isComplete)
        {
            ...
            MissionTargetType targetType = missionTargetInProgress.TargetType;
            Character character = missionInProgress.character;
            ...
            switch (targetType)
            {
                case MissionTargetType.loot_item:
                    ...
                    int lootItemsCount = info.GetOrDefault<int>(k.progressCount);
                ...
                case MissionTargetType.kill_definition:
                    ...
                    int killDefinitionCount = info.GetOrDefault<int>(k.progressCount);
                ...
                case MissionTargetType.dock_in:
                    int zoneId = info.GetOrDefault<int>(k.zoneID);
                    Position position = info.GetOrDefault<Position>(k.position);
                    ec = missionTargetInProgress.AdvanceTarget_DockIn(position, zoneId);
                    break;

                case MissionTargetType.use_itemsupply:
                    int itemCount = info.GetOrDefault<int>(k.progressCount);
                ...
                case MissionTargetType.massproduce:
                    ...
                    int producedDefinition = info.GetOrDefault<int>(k.definition);
                    int quantity = info.GetValue<int>(k.quantity);
                ...
                case MissionTargetType.prototype:
                    ...
                    int protoDefinition = info.GetOrDefault<int>(k.definition);
                ...
                case MissionTargetType.summon_npc_egg:
                    int currentProgress = info.GetOrDefault<int>(k.progressCount);
                ...
                case MissionTargetType.fetch_item:
                    ...
                    int quantityFetched = info.GetOrDefault<int>(k.progressCount);
                    int location = info.GetOrDefault<int>(k.location);
                    ec = missionTargetInProgress.AdvanceTarget_FetchItem(isComplete, quantityFetched, location);
                    break;

                case MissionTargetType.lock_unit:
                    int lockedUnitCount = info.GetOrDefault<int>(k.progressCount);
                    long[] lockedNpcEids = info.GetOrDefault<long[]>("lockedUnits");
        ...
        {
            Command gangResultCommand = Commands.MissionTargetUpdate;
            ...
            IList<Item> rewarditems = TryFinishMission(missionInProgress);
            ...
            {
                Dictionary<string, object> statusInfo = missionInProgress.PrepareInfoDictionary(message: gangResultMessage, sourceCharacterId: doerCharacter.Id);
        ...
        private IList<Item>? TryFinishMission(MissionInProgress missionInProgress)
        {
            ...
            {
                //mission is finished
                Mission mission = missionInProgress.myMission;

                MissionBonusObjects.MissionBonus missionBonus = MissionAdministrator.GetOrAddMissionBonus(missionInProgress.character, mission.missionCategory, missionInProgress.MissionLevel, missionInProgress.myLocation.Agent);

                Logger.Info("++ Mission is finished " + missionInProgress);

                Dictionary<string, object> successData = new()
                {
                ...
                IList<Item> rewardItems = missionInProgress.SpawnRewardItems(out MissionLocation locationUsed);

                //online gang members
                List<Character> charactersToInform = missionInProgress.GetAffectedCharacters();

                //mission participants
                List<Character> participants = missionInProgress.GetParticipantsVerbose().ToList();

                participants = missionInProgress.FilterParticipants(participants);

                int ep = missionInProgress.CalculateEp();

                Dictionary<string, object> epDict = new();
                int count = 0;
                foreach (Character? participant in participants)
                {
                    ...
                    Dictionary<string, object> oneEpEntry = new()
                    {
                ...
                bool isTempTeleportEnabled = false;
        ...
        {
            foreach (Character character in charactersToSpread)
            {
                MissionBonusObjects.MissionBonus missionBonus = MissionAdministrator.GetOrAddMissionBonus(character, missionInProgress.myMission.missionCategory, missionInProgress.MissionLevel, missionInProgress.myLocation.Agent);
        ...
        {
            List<KeyValuePair<MissionTargetInProgress, MissionInProgress>> resultList = new();
            ...
            foreach (MissionInProgress missionInProgress in collector.GetMissionsInProgress())
            {
                MissionTargetInProgress[] list = missionInProgress.CollectIncompleteTargetsByType(targetType).ToArray();
                ...
                foreach (MissionTargetInProgress? missionTargetInProgress in list)
                {
        ...
        public void NpcGotKilledInAway(Character character, Guid guid, Dictionary<string, object> data)
        {
            ...
            MissionTargetInProgress? targetInProgress = missionInProgress.CollectIncompleteTargetsByType(MissionTargetType.kill_definition).FirstOrDefault(t => t.IsMyTurn);
            ...
            bool isComplete = targetInProgress.progressCount + 1 == targetInProgress.myTarget.Quantity;
