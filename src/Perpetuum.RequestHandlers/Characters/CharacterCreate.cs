...
namespace Perpetuum.RequestHandlers.Characters
{
    public class CharacterCreate : IRequestHandler
    {
        ...
        private readonly TimeSpan WAIT_TIME_BEFORE_SENDING_MAIL = TimeSpan.FromSeconds(10);
        private readonly TimeSpan WAIT_TIME_BEFORE_SENDING_WELCOME_MESSAGE = TimeSpan.FromSeconds(10);
        ...
        public void HandleRequest(IRequest request)
        {
            ...
            int raceID = request.Data.GetOrDefault<int>(k.raceID);
            int majorID = request.Data.GetOrDefault<int>(k.majorID);
            int schoolID = request.Data.GetOrDefault<int>(k.schoolID);
            int sparkID = request.Data.GetOrDefault<int>(k.sparkID);
            ...
            int activeCharactersCount = _accountManager.GetActiveCharactersCount(account);
            ...
            if (schoolID > 0)
            {
                ...
                IEnumerable<Extension> extensions = character.GetDefaultExtensions();
                character.SetExtensions(extensions);

                int sparkToActivate = _sparkHelper.ConvertCharacterWizardSparkIdToSpark(sparkID);
        ...
        {
            Entity characterEntity = _entityServices.Factory.CreateWithRandomEID(DefinitionNames.PLAYER);
            _entityServices.Repository.Insert(characterEntity);

            int id = Db.Query().CommandText("insert into characters (rootEid) values (@rootEid);select cast(scope_identity() as int)")
