...
namespace Perpetuum.RequestHandlers.Channels
{
    public class ChannelCreateForTerminals : IRequestHandler
    {
        ...
        public void HandleRequest(IRequest request)
        {
            ...
            foreach (DockingBase dockingBase in _dockingBaseHelper.GetDefaultDockingBases())
            {
                int count = Db.Query().CommandText("select count(*) from channels where name=@terminalChannelName")
