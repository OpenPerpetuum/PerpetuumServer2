...
each build of `Perpetuum.Server` (requires .NET 10 SDK + `dotnet tool restore` from the
solution root).
...
Communities (Louvain clustering, 2 594 clusters) group related symbols.
...
under "God Nodes". These are the highest-risk files to change: `Character.cs`, `Player.cs`,
`MissionInProgress.cs`, `Outpost.cs`, `AdminCommandHandlers.cs` and similar.
