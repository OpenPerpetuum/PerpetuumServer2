...
namespace Perpetuum.Accounting.Characters
{
    ...
    public class Character : IEquatable<Character>, IComparable<Character>
    {
        ...
        public static Character None => none ??= CharacterFactory(0);
        ...
        public Position? ZonePosition
        {
            get
            {
                ...
                return x == null || y == null
                    ? null
                    : new Position((double)x, (double)y);
            }
            set
            {
                double? x = null;
        ...
        public long AllianceEid
        {
            ...
            set => WriteValueToDb(CharacterConstants.FIELD_ALLIANCE_EID, value == 0L ? null : value);
        }

        public long ActiveRobotEid
        {
            get => ReadValueFromDb<long?>(CharacterConstants.FIELD_ACTIVE_CHASSIS) ?? 0;
            set => WriteValueToDb(CharacterConstants.FIELD_ACTIVE_CHASSIS, value == 0L ? null : value);
        }
...
WHERE characterid=@characterID AND e.active = 1 AND e.hidden = 0")
        ...
        public int GetExtensionLevelSummaryByName(params string[] extensionNames)
        {
            ...
            return extensionReader.GetExtensionIDsByName(extensionNames).Sum(ex.GetLevel);
        }
        ...
        public IDictionary<string, object> GetFullProfile()
        {
            ...
            Dictionary<string, object> profile = new(21)
            {
                ...
                {k.lastUsed, record.IsDBNull("lastused") ?  null : record.GetValue<DateTime>("lastused")},
                {k.rootEID, record.GetValue<long>("rootEID")},
                {k.totalMinsOnline, record.GetValue<int>("totalMinsOnline")},
                {k.activeChassis, record.IsDBNull("activeChassis") ?  null : record.GetValue<long>("activeChassis")},
                {k.baseEID, record.IsDBNull("baseEID") ?  null : record.GetValue<long>("baseEID")},
                ...
                {k.lastLogOut, record.IsDBNull("lastLogOut") ?  null : record.GetValue<DateTime>("lastLogOut")},
