namespace Perpetuum.Zones.LootContainers
{
    ...
    public static class LootItemRepositoryExtensions
    {
        ...
        {
            LootItem? f = repository.GetByDefinition(container, lootItem.ItemInfo.Definition).FirstOrDefault(l => Math.Abs(l.ItemInfo.Health - lootItem.ItemInfo.Health) < double.Epsilon);
            if (f == null)
            {
